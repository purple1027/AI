...
cy.rent <- c(92510,74801,27168,33929,89807,91253,92045,75328,48756,21875)
plot(hy,cy.rent, col='red',pch=19)
...
plot(years,popu,main='예상인구수',type='b',col='blue')
...
plot(year,male, type='b', col='blue', main='남녀 경제활동 참가율', ylim =c(50,75))
...
plot(Orange$age, Orange$circumstance, col=color)

# 12.Orange# 12. mlbench 패키지에서 제공하는 Glass 데이터셋에 대해 다음의 문제를 해결하기 위한 R 코드를 작성하시오.
...
colors <- c('yellow','pink','red','purple','orange','green','gray','blue','#99aa00','#88ee11')
...
plot(year, hlevels, type='b', col='blue', ylab='수위', main='LakeHuron')
...
plot(months, year.1949, main = '연도별 항공 승객 수', type = 'b', col='red',ylim=c(100,650))
...
plot(months, deaths.1973, ylim = c(6500,12000), col='red', main='사고 사망자 수', ylab = '사망자수', xlab = '월' , type='b')
lines(month, deaths.1975,type='b', col='black')
lines(month, deaths.1977,type='b', col='purple')
